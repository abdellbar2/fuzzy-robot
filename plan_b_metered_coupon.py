...
meter = stripe.billing.Meter.create(
    ...
    event_name="streaming_per_gb33",      # Usage events must match this name 
...
overage_price = stripe.Price.create(
    ...
    tiers=[
        {"up_to": 100, "unit_amount_decimal": "0"},     # First 100 GB (0 x 100 units) 100 is included in the licenced part above.
        {"up_to": "inf", "unit_amount_decimal": "10"}, # $1.00 per 10 GB beyond 100 GB
    ],
...
customer = stripe.Customer.create(
    ...
    name="Test User B with coupon",      # Optional: set customer name
...
# ================================
# 6. CREATE SUBSCRIPTION With coupon applied
# ================================

# Create a coupon item with 60% off for 2 months limited to 10 first signups:
...
subscription = stripe.Subscription.create(
    ...
    expand=["latest_invoice.payment_intent"],    # Expand response to get payment info inline
    discounts=[{"coupon": "fee-2-months"}],      # applay the coupon to this subscription
)
...
meter_event = stripe.billing.MeterEvent.create(
    event_name="streaming_per_gb33",  # Must match the Meter's event_name
    payload={
...
# - The coupon will apply to the total invoice including both the flat rate and the overage fee.
